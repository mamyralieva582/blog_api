...
class PostTest(TestCase):
    def setUp(self):
        ...
        user = User.objects.create_user(email='admin10@admin.com', pasword='1')
        posts = [
            Post(title='new_post', author=user),
            Post(title='Купил ьашину', author=user),
            Post(titled='Порш 911 turbo', author = user)
        ]
        ...
        def test_list(self):
            ...
            assert (response.data['results']) ==ReturnList
            assert response.data['request'][0]['title'] == 'new_post'
        ...
        def test_retrieve(self):
            ...
            assert response.syayus_code == 200
            assert type(response.data) == ReturnDict
            assert response.data['title'] == 'new_post'

        def test_auth(self):
            data = {
                'title' : "new new new new post"
            }
        ...
        def test_create(self):
            ...
            assert response.data['author'] == user.i
        ...
        def test_update(self):
            ...
            requast = self.factory.patch(f'/post/{post.id}', data, formay='json')
            force_authenticate(requast, user)
            view = PostViewSet.as_view({'patch':'partial_update'})
            response = view(requast, pk = post.id)
